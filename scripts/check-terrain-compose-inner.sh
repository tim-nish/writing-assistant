...
#   Assertion content unchanged; the eight inline fragments now share two
...
# Fixture prep — ONE run builds both derived maps (#950): the thin map for the
# free-form-every-time assertion, and the over-budget map for the size switch.
python3 - "$work/map.json" "$work" <<'PYEOF'
import json, sys
w = sys.argv[2]

thin = json.load(open(sys.argv[1]))
thin["topics"] = thin["topics"][:1]
for t in thin["topics"]:
    t["subtopics"] = t["subtopics"][:1]
json.dump(thin, open(w + "/thin-map.json", "w"))
...
c = json.load(open(w + "/cands.json"))["candidates"]
...
p = json.load(open(w + "/payload.json"))
check(len(p["items"]) == 1, "it is ONE screen, not a sequence of them")
item = p["items"][0]
labels = [c2["label"] for c2 in item["choices"]]
...
thin_labels = [c2["label"] for c2 in
               json.load(open(w + "/thin-payload.json"))["items"][0]["choices"]]
check(any("name your own" in l for l in thin_labels) and "stop here" in thin_labels,
      "free-form is offered every time, not only on rejection")

# --- the over-budget summary screen ------------------------------------------
p = json.load(open(w + "/big-payload.json"))
view = open(w + "/view.md", encoding="utf-8").read()
d = json.load(open(w + "/big-map.json"))
check(len(p["items"]) == 1, "the over-budget screen is still ONE screen")
item = p["items"][0]
check(w + "/view.md" in item["where"],
...
spec = importlib.util.spec_from_file_location("dvtext", tpath)
...
strands = [x for x in json.load(open(w + "/big-cands.json"))["candidates"]
           if x["kind"] == "element"]
check(len(strands) > 3,
