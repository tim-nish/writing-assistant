...
# --- Cache-population compliance gate (#534) — RETIRED (Story 20.147, #1182) -
# The gate keyed harvest reads to `harvest-cache.py` entries; harvest and its
# cache are retired, so there is no population step left to enforce and the
# helper module is gone with it. The `--check-cache-population` flag is removed
# rather than stubbed: a caller still passing it should fail loudly, not be
# reassured by a vacuous pass.
