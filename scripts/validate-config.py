...
# `_hub_topic_names()` and `_articles_backlog_tracks()` were REMOVED (Story
# 20.161, #1246) with the `track_topics` mapping whose existence lint and
# stale-mapping warning were their only consumers (#525).


# `validate_journey()` was REMOVED (Story 20.134, #1183) with the `journey:`
# key it linted: there is no block left to be malformed, and a lint over a key
# nothing reads reports a defect with no consequence.
...
def validate_policy_source(args, findings, notices):
    ...
    The `track_topics` topic-existence lint and stale-mapping warning (#525,
    Story 18.33) were REMOVED with the mapping itself (Story 20.161, #1246):
    there is no key left to lint."""
    cmd = [sys.executable, SRC_RES]
    if args.root:
        cmd += ["--root", args.root]
    cmd += ["policy-source"]
