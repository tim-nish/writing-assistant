...
git repo the script errors rather than silently resolving against cwd):
  ...
  policy-source             Print the optional `policy_source` block as JSON:
                            ...
                            or the RETIRED `path` key (13.73) — exits 4 with
                            ...
                            The `track`/`topics` keys, their validation, and
                            the `track_topics` mapping (#525) were REMOVED by
                            Story 20.161 (SPEC-policy-topic-at-draft CAP-3/
                            CAP-5, amended 2026-08-02, #1246) once no owned
                            repo config carried them; the parser no longer
                            knows those keys.
  ...
  set-policy-source [--disable]
                            ...
                            The `--track-topics` mapping writer (#525) was
                            removed with the mapping it wrote (Story 20.161).
...
# 6 was JOURNEY_MALFORMED, RETIRED with the `journey:` key itself (Story
# 20.134, #1183). The key existed only to feed the host-repo episode join, and
# that join is removed; the number is left unreused so an old caller reading a
# 6 finds nothing pretending to be its old meaning.
...
def set_policy_source(lines, enabled=True):
    ...
    for a legacy `path:` / `track:` / `topics:` / `track_topics:` block);
    comments outside it do. With enabled=False the block is removed entirely
...
# `get_journey()` was REMOVED (Story 20.134, #1183). It parsed the optional
# top-level `journey:` block — episode-record files inside the declared source
# set — whose ONLY consumer was `terrain_map.py`'s host-repo episode join. That
# join asserted an absence over whatever text these files happened to contain
# (zero bytes where the block was absent) and could only ever be satisfied by
# the recording IOU written for it, so key and join are retired together.
# A repository that wants such a file read still declares it as an ordinary
# `sources:` path — that is the read path it always went through.
...
# `_parse_track_topics`, `validate_track_topics` and `render_track_topics`
# (#525) were REMOVED by Story 20.161 (SPEC-policy-topic-at-draft CAP-5,
# amended 2026-08-02, #1246): the mapping existed only to seed the retired
# per-article ≤2-topic proposal, and no owned repo config carried it.
...
def get_policy_source(lines, root):
    ...
    pairs for a malformed block. The RETIRED `path` key (13.73) is a named
    configuration error carrying a migration notice — NEVER silently honored.
    The `track`/`topics` keys, their validation, and the `track_topics`
    mapping (#525) were removed by Story 20.161 (CAP-3/CAP-5 executed once no
    owned repo config carried them): the parser no longer knows those keys,
    so a leftover line is simply unread, like any other unknown key.
    ...
    while j < len(lines):
        ...
        if re.match(r"^\s+path:", ln):
            ...
            continue
        # `track:` / `topics:` / `track_topics:` are gone entirely (Story
        # 20.161, SPEC-policy-topic-at-draft CAP-3/CAP-5 as amended 2026-08-02,
        # #1246): the keys, their validation, and their writer flags were
        # removed together once no owned repo config carried them — no
        # permanent dead compat path. A leftover line is unknown to this
        # parser and unread.
...
def cmd_set_policy_source(args):
    # The `--track-topics` mapping writer (#525) was removed with the mapping
    # it wrote (Story 20.161): the writer records the presence toggle only.
    import json
    root = host_root(args.root)
    lines = read_lines(root)
...
def cmd_files(args):
    ...
    files = sorted(enumerate_files(sources))
    for f in files:
        print(f)
...
def main(argv=None):
    ...
    sp.add_argument("--disable", action="store_true",
                    help="remove the policy_source block entirely "
                         "(same semantics as never declaring it)")
    # The `--track-topics` mapping writer (#525) was removed by Story 20.161
    # together with the `track_topics` config key it wrote.
