...
def _cost_proxies(ws):
    """The run's cost proxies (#742) — the journal's own reader owns them
    (relocated with story 20.195: this file is at its line ratchet and the
    sanctioned remedy is moving code to the module that owns it)."""
    return run_record.cost_proxies(ws)
...
def cmd_complete(args):
    ...
    if (_bare := _load("product_citation.py").complete_refusal(text)):  # backstop #1339
        return product_error("canonical draft (drafts/{slug}.md)", args.draft, _bare)
    try:
...
# The review command family lives in its own module (Story 20.45, #919). It
# borrows shared helpers from here, so the host binds itself once and the
# commands are re-exported under their original names — the dispatcher, the
# parser and every caller are untouched, which is what keeps the CLI surface
# byte-identical.
sys.path.insert(0, os.path.dirname(os.path.realpath(__file__)))
import draft_review  # noqa: E402
draft_review.bind(globals())
cmd_review_consulted = draft_review.cmd_review_consulted
cmd_review_reentry = draft_review.cmd_review_reentry
cmd_review_checkpoint_proposal = draft_review.cmd_review_checkpoint_proposal
cmd_review_diff = draft_review.cmd_review_diff

# The policy-classification family follows the same shape (Story 20.46, #920):
# same bind-the-host pattern, same byte-identical CLI surface.
import draft_policy  # noqa: E402
draft_policy.bind(globals())
cmd_classify_policy = draft_policy.cmd_classify_policy
cmd_policy_block_check = draft_policy.cmd_policy_block_check
cmd_policy_prefilter = draft_policy.cmd_policy_prefilter

# The variants and quality-gate family follows the same shape (Story 20.47,
# #921). Its helpers stay HERE deliberately: checks exec this file and call
# them as module attributes, or assert on them at source level (see the
# draft_variants.py docstring) — only the two command bodies move.
import draft_variants  # noqa: E402
