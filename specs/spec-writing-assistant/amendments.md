...
> **Amended 2026-08-04 (triage, #1367) — a change that ships a CONTROL SURFACE names its operating doc in the same change, and the obligation lands as a CLAUSE PLUS A VISIBLE-ABSENCE SIGNAL rather than either alone.** #1360 shipped block mode with no mention in any file the agent driving a run reads: the mechanism worked, the call sites existed, and the invocation-site carrier passed green — because reachability is **call-site AND a guard that can be true**, and the second conjunct is data-dependent and invisible to enumeration. Block mode's guard became true only via `run_block.py enable` or an environment variable, and nothing instructed the agent to perform either act, so the guard was effectively constant-false on every skill-driven run. **A passing audit and a dark feature were the same observation.**
>
> **The remedy is generation-side, and the served surface settles the shape the issue left open.** #1367 asked whether a definition-of-done clause is the whole remedy. It is not: an obligation is violated by an absence, which produces no event to hook, so it *"stays behavioral and its carrier is a signal that makes the absence visible"*, and the same-shaped case ruled that a contract amendment and a lint land **together** — *"the staged 'or' is false"* — on the evidence that prose alone had been observed insufficient on the surface best positioned to know better *(owner decision record — 2026-07-23 (carry a rule at its violation layer); owner decision record — 2026-07-26 (a prohibition without a gate becomes an obligation plus a visible-absence signal))*.
>
> **The second open question dissolves rather than being answered.** "What counts as a control surface?" needs no global definition, because the surface **declares itself**: a module shipping one carries a `# control-surface:` header naming how it is enabled and which operating doc carries that affordance — the same greppable-header shape `# tier:`, `# covers:` and `# removal-signal:` already use, which 95 files in `scripts/` demonstrate authors reach. The signal then asserts over **declared** surfaces only. Defining the class globally would be the enumeration problem one level up, and this repository has four recorded instances of coverage being relative to its enumeration; a declaration inverts that — an undeclared surface is the author's omission, not the checker's blind spot, and it is exactly the omission the ambient clause binds.
>
> **The clause is ambient, never skill-sited.** It is broken at the moment a change introduces a surface, in any sitting including one that does not present as capability work, and *"a skill binds only the sittings that invoke it"*. So `CLAUDE.md` carries the rule.
>
> **No new check joins the family.** The signal extends `scripts/check-subcommand-carriers.sh`, which already guards the *"mechanism built, orchestration missing"* class — #1367 is that same class one conjunct further on, so this is assertion altitude rather than family growth, which is the sanctioned lever.
>
> **What would overturn this:** authors not reaching the declaration in practice — a control surface shipping with no `# control-surface:` header and nobody noticing, which the ambient clause is supposed to prevent and the signal cannot detect (it asserts over declarations, and an absent declaration is invisible to it by construction). That is the residual, stated rather than hidden: the signal covers declared surfaces, and the clause covers whether they get declared. Delivery: story 20.214.
