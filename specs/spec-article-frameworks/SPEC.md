...
> **Amended 2026-07-29 (triage, #911) — F1–F5 demote from MOLD to CANDIDATES, with their removal signal recorded at the demotion; and F2's mandatory-surprise gate retires now.** Under the thesis model an article's structure is proposed **from the material** — Strands, Journeys and the facts grounding them — so a framework is an option the proposer may match or ignore, never a form to fill. That is the demotion. **The removal signal is recorded here, at the moment of the change, per the ratified provisional-adoption discipline** (a mechanism is judged on real use and removed if it fails to deliver value, never preserved merely because it was implemented): **window** — five accepted articles under the thesis model, or 2026-10-31, whichever comes first; **falsifier** — accepted structures are routinely bespoke, or framework-matched ones are routinely rewritten; **default at expiry — REMOVE.** Keeping F1–F5 past the window requires evidence, not the absence of it. **The instrument is corrected before it is adopted, and this is the load-bearing part of the amendment.** The proposal was to record, per article, whether the accepted structure was framework-matched. But the failure mode under watch is *the proposer ignores the frameworks entirely* — and a proposer that ignores them generates no framework-match event, so an instrument keyed on match events falls silent exactly as the hypothesis fails, with its silence indistinguishable from success. The served rule names this shape precisely: *"a provisional design's tripwire must be expressible in acts the design can already perform … a tripwire whose firing requires the capability the failure removes gets quieter exactly as the failure gets worse"* (*owner decision record — 2026-07-27 (a tripwire cannot need the missing capability)*). **So the field is recorded on EVERY accepted structure, carrying an explicit `bespoke` value when no framework was matched** — silence becomes impossible, and an accepted structure with no row is a *missing measurement* rather than a passing one. It stays free: the proposal already knows its own provenance, so this is a plan-level field and no new store. **F2's mandatory surprise retires immediately, independent of the window.** Its `GATE (entry)` required "at least one real surprise/failure with an artifact you can show", and an unfilled GATE slot makes a draft unpublishable — so a required surprise in every article is a publishability bar the material cannot always clear, and a surprise produced to clear a bar is not one. It survives as **one narrative kind among the several the gap vocabulary already carries**, used when the material genuinely holds one. This half is not provisional and carries no window: it is a correction, not a hypothesis.
