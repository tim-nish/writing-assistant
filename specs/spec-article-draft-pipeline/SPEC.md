...
- Durability under a turn ceiling (added 2026-07-12): wall-clock is unconstrained but the turn/compute budget is not — the pipeline persists per-stage checkpoint state to the run workspace (`$WS`, building on the intermediates invariant below) and a re-invocation resumes from the last completed stage, so a turn-ceiling casualty is recoverable rather than a total loss. Each stage surfaces a budget-triage signal before hard failure; resumption is a stated contract, not an agent default.
