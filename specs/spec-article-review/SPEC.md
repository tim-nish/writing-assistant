...
> **Amended 2026-07-26 (triage, #751/#757)** per /triage-gh on two re-entry freshness defects (run 20260726T165310). **(1) The versioned verdict record is sha-bound (#751):** `rubric-verdicts-v2.txt` (the #496 completeness contract) proved acceptable while stale — re-entry checkpointed `done/reviewed` while the most recent gate run on that exact draft had failed, because the record was complete but computed from an older draft version. The v2 record now opens with the same `attestation: draft-sha256=<hex>` convention the provenance verdicts carry, written by `quality-gate --verdicts-out`, and **re-entry with `--rubric-applied` refuses (non-zero, no checkpoint) a record whose sha does not match the edited draft** — completeness and freshness are now both mechanical. **(2) Re-entry re-projects the article plan (#757):** an arbitration round that applies ≥1 edit can change exactly the facts `plans/<slug>.md` exists to carry (observed: the plan's `audience:` contradicting the reviewed canonical after an owner-directed narrowing), and the post-arbitration step list wrote everything *except* the plan. The step list now includes **re-emitting the plan through the same sanctioned writer** (`write-article-plan.py write`, fail-closed validation, conformance re-recorded) as a deterministic projection of the reviewed canonical + the round's arbitration record — no new owner interaction, per SPEC-article-plan CAP-1 as amended the same date. Rationale: the post-review revision entry point reconstructs the writing context from the plan alone, so a stale plan is not thin but wrong (owner decision record — 2026-07-26 (revise entry point; plan freshness)). Enforced at `cmd_review_reentry` (`scripts/draft-pipeline.py`) and the re-entry steps in `skills/review-article/SKILL.md`, in lockstep.
...
- **Severity is criterion-anchored** (added 2026-07-11 per `docs/harness-architecture.md` D3): a finding is a blocker only by naming its criterion — a quality-rubric dimension violation (the draft pipeline's stage 3→4 gate rubric, re-checked here as the second net), a cold-read Q1/Q2 mismatch, or a configuration defect. **A fourth criterion applies to derived canonicals only** (added 2026-07-27, #800): a **declared-convention violation** — prose diverging from the `register` or `terminology` its language declares in `config/language-conventions.yaml`. It is deliberately artifact-class-scoped: a draft carrying `adapted_from` (`scripts/draft-pipeline.py:2586`) is graded against its declared conventions because SPEC-canonical-adaptation CAP-4 makes that the language half of its review scope, while an authored EN canonical keeps exactly the three criteria above and the closed four-dimension rubric. This widens *what a blocker may name* by one class, not *for whom*: the anchor holds only while the scoping does, so a proposal to extend this criterion beyond derived canonicals is a spec change, never an implementation detail. Rubric-mapped structure/prose findings are blocker-eligible, and an open one blocks the "publishable" verdict exactly as configuration blockers do. The full severity criteria table lives in `review-prompts.md`; severity assigned without a named criterion is a contract violation, not reviewer judgment.
...
- **Post-arbitration re-entry** (added 2026-07-18, #362): an arbitration round that applied edits does not end the run at the edit — it re-enters the verification regime before anything is reported done: **persist the reviewed canonical** to its declared product path (`drafts/{slug}.md`, SPEC-article-draft-pipeline), **rebuild and revalidate the provenance map** against the edited draft (review-authored sentences are classified like any other — the zero-unmarked-claims guarantee survives review), **run scoped regression checks on the applied edits** (verify-provenance on the rebuilt map; the quality gate's mechanical dimensions where a rubric-mapped finding was applied), **mark existing variants stale** (SPEC-platform-variants CAP-6), and **stop**. **The evidence is typed by artifact class (amended 2026-07-25, #704):** the map-and-verify-provenance regime above is the **authored canonical's**. A **derived canonical** — one carrying `adapted_from` — owns no claims of its own (they are inherited), so it has no provenance map by design and re-enters on **ancestry evidence** instead; that path is defined once in **SPEC-canonical-adaptation CAP-4** and is not restated here. Everything else in this constraint binds both classes identically, the closing rule below included — read as "the steps of its class". Review never emits or re-emits a variant — re-emission is a fresh explicit publish decision (SPEC-platform-variants CAP-3). A run that skips any of these steps may not report the draft "publishable".
