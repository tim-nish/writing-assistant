...
- **The Brief selection gate (story 20.192, #1343; SPEC-terrain amendments, the 2026-08-03 block, clause (b)).** With **no** `--brief`, stage 0 enumerates this repository's Brief home and returns `brief_selection_required` with the gate payload — **present it exactly as emitted** (nothing pre-selected, the free-form override intact), then re-invoke stage 0 with the chosen Brief's path as `--brief`, or start cold; **nothing is checkpointed until the Brief is known**, and an empty home returns `brief_selection` stating the absence and proceeds cold.
- **Workspace autostart** — resumption is **automatic, not opt-in**; on a `--brief` invocation automatic means **same-brief-only** (amended 2026-08-02, #1207 — the amendments companion is the authority: a different-brief run is skipped fresh via `fresh_skipped`, nothing deleted; cold invocations unchanged below; the binding is run identity carried at the one checkpoint write path and a rebinding write is refused, and the read is THREE-valued — *no binding recorded* is handed to the owner as a confirmation, never collapsed into *different*, story 20.193/#1338). It resumes
