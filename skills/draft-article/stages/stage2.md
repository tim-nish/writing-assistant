...
policy-seeded tension (or reconciliation) question when one exists, since it
reframes every later answer — then the rest in selection order. The old
`opinion`/`audience`/`significance`/color ranking went with the question bank
(Story 20.131, #1147) and ranked questions that no longer exist. Batching
within that order is free — grouping several questions into one ask is fine —
but never reorder across it. The journal echoes the order, so a mis-ordered run
is attributable. Selection priority (the bullets above) is unchanged; this
governs presentation only. When harvest
