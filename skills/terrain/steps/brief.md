...
## After the thesis: three more gates, in [`brief-gates.md`](brief-gates.md)

Once a thesis is adopted the brief carries three further selections, each the
same mechanism one step later and each read in that companion: **journey
incorporation** (how served arcs enter the article, #1045), **structure**
(composed for THIS article, never selected from a stock, #1410), and **plain
register** (the child-level commitment both article ends realize, #1411).
They moved there when this file crossed its companion ceiling; the contract is
unchanged and lives in one place.
